...
public final class AutoTrap extends Module
{
    ...
    public final Value<String> Mode = new Value<>("Mode", new String[] {"Mode"}, "The mode to use for autotrap", "Full");
    ...
    public void onEnable()
    {
        ...
        {
            Summit.SendMessage(String.format("[AutoTrap] You do not have any %s in your hotbar!", ChatFormatting.LIGHT_PURPLE + (EChests.getValue() ? "Ender Chests" : "Obsidian") + ChatFormatting.RESET));
            toggle();
        }
    }
    ...
    private Listener<EventPlayerMotionUpdate> OnPlayerUpdate = new Listener<>(p_Event ->
    {
        ...
        if (closestTarget == null)
        {
            if (firstRun)
            {
                ...
                if (announceUsage.getValue())
                {
                    Summit.SendMessage("[AutoTrap] Enabled, waiting for target.");
                }
            }
            return;
        }
        if (firstRun)
        {
            ...
            if (announceUsage.getValue())
            {
                Summit.SendMessage("[AutoTrap] Enabled, target: " + lastTickTargetName);
            }
        }
        else if (!lastTickTargetName.equals(closestTarget.getName()))
        {
            ...
            if (announceUsage.getValue())
            {
                Summit.SendMessage("[AutoTrap] New target: " + lastTickTargetName);
            }
        }
