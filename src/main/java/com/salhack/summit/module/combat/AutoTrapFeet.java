...
public class AutoTrapFeet extends Module
{
    ...
    public void onEnable()
    {
       ...
       //         .orElse(null);
        ...
        if (Target != null)
        {
            Summit.SendMessage("[AutoTrapFeet]: Found target " + Target.getName());
        }
    }
        ...
        if (hasStack(Blocks.OBSIDIAN) || slot != -1)
        {
            if ((mc.player.onGround && playerSpeed <= 0.005f))
            {
                ...
                for (BlockPos l_Pos : l_Array)
                {
                    ...
                    {
                    //    Summit.SendMessage(String.format("Can't place at %s because of %s", l_Pos.toString(), l_ResultPlace.toString()));
                    }
                }
