...
public final class ShulkerPreview extends Module
{
    ...
    { "Server Mode" }, "Server mode, like 2b2t has custom preview settings", "Normal");
    ...
    public ShulkerPreview()
    {
        ...
        Mode.addString("DropPacket");
        Mode.addString("Inventory");
    }
    ...
    {
        if (!Mode.getValue().equals("DropPacket"))
            return;
    ...
    private Listener<EventServerPacket> onServerPacket = new Listener<>(event ->
    {
        ...
        {
            final SPacketSetSlot l_Packet = (SPacketSetSlot) event.getPacket();
    ...
    private Listener<EventRenderTooltip> OnRenderTooltip = new Listener<>(p_Event ->
    {
        ...
        if (Item.getIdFromItem(p_Event.getItemStack().getItem()) == 130)
        {
            ...
            // text
            RenderUtil.drawStringWithShadow("Summit EnderChest Viewer", 0, -mc.fontRenderer.FONT_HEIGHT - 1,
                    0xFFFFFFFF);
            ...
            if (this.middleClick.getValue())
            {
                if (Mouse.isButtonDown(2))
                {
                    if (!this.clicked)
                    {
                        InventoryBasic l_Inventory = new InventoryBasic("Summit EnderChest Viewer", true, 27);
